...
def generate_discussion(summary: dict, failure_modes: dict) -> str:
    ...
    return (
        "This benchmark compares a single-attempt ReAct baseline against a multi-attempt "
        "Reflexion agent on the same dataset. Reflexion typically helps when the first answer "
        "misses a second hop or drifts to the wrong entity, because the next attempt can reuse "
        "reflection memory to course-correct. In this run, ReAct EM was "
        f"{react.get('em', 0)} while Reflexion EM was {reflexion.get('em', 0)}, for a delta of "
        f"{delta.get('em_abs', 0)}. The improvement comes with extra cost: average attempts, "
        "tokens, and latency all tend to increase for Reflexion because the evaluator and "
        "reflector add work after failed tries. ReAct failure modes in this run were "
        f"{react_failures}, while Reflexion failure modes were {reflexion_failures}. These "
        "numbers make the tradeoff visible: reflection can recover some reasoning mistakes, but "
        "its gains still depend on evaluator reliability and the quality of the strategy fed into "
        "the next attempt."
    )
...
def build_report(
    ...
    return ReportPayload(
        ...
        discussion=discussion or generate_discussion(summary, failure_modes),
    )
